...
function EbN0_BER_QAM_approx_nofec( ...
    logdata, logdata_fxp8, logdata_fxp8_base, logdata_fxp8_approx,...
    ...
    refdata = readtable("data/approx/log_flp_nofec_qam16/log_data.txt");
    ...
    for i=1:length(iter)
        ...
        semilogy(y_fxp16(:,1), y_fxp16(:,2) + eps, ...
            ...
            "Color", navy_blue,...
            "DisplayName", "FXP S2.14 Base (MUL16)");
        hold on;
    end
    ...
    for i=1:length(iter)
        ...
        semilogy(y_fxp8(:,1), y_fxp8(:,2) + eps, ...
            ...
            "Color", steel_blue,...
            "DisplayName", "FXP S2.14 Approx (MUL16)");
        hold on;
    end

    % Add FXP Base model (S2.6)
    % Extract data from table for specific Modulation and LDPC iterations
    idx_3 = find(logdata_fxp8_base.Modulation == modul & logdata_fxp8_base.LDPC_Iter == iter(i));
    
    % Create array with EbN0dB noise and BER value
    y_fxp8_base(:,1) = logdata_fxp8_base.EbN0dB(idx_3);
    y_fxp8_base(:,2) = logdata_fxp8_base.BER(idx_3);

    % Plot BER relative to Eb/N0
    semilogy(y_fxp8_base(:,1), y_fxp8_base(:,2) + eps, ...
        "--v", "MarkerSize", 6,...
        "Color", purple,...
        "DisplayName", "FXP S2.6 Base (MUL16)");
    hold on;

    % Add FXP Approximate model (S2.6)
    % Extract data from table for specific Modulation and LDPC iterations
    idx_4 = find(logdata_fxp8_approx.Modulation == modul & logdata_fxp8_approx.LDPC_Iter == iter(i));
    
    % Create array with EbN0dB noise and BER value
    y_fxp8_approx(:,1) = logdata_fxp8_approx.EbN0dB(idx_4);
    y_fxp8_approx(:,2) = logdata_fxp8_approx.BER(idx_4);

    % Plot BER relative to Eb/N0
    semilogy(y_fxp8_approx(:,1), y_fxp8_approx(:,2) + eps, ...
        "-.diamond", "MarkerSize", 6,...
        "Color", indigo,...
        "DisplayName", "FXP S2.6 Approx (MUL16)");
    hold on;
